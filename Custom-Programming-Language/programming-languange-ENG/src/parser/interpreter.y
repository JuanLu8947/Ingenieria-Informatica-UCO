...
%type <st> stmt asgn print read if while block repeat for dowhile readstring clearscreen place text switch
...
dowhile:  DO controlSymbol LETFCURLYBRACKET stmtlist RIGHTCURLYBRACKET WHILE cond SEMICOLON
		{
			lp::BlockStmt *aux = new lp::BlockStmt($4); //bloque aux de statements para el bloque de DOWHILE

			// Create a new dowhile statement node
			$$ = new lp::RepeatStmt(aux, $7);
...
switch:	SWITCH controlSymbol cond cases ENDSWITCH   /* LP exp RP */
		{
			$$ = new lp::SwitchStmt($3, $4); //crea un nuevo nodo switch
		
			control--;
		}
		| SWITCH controlSymbol cond cases DEFAULT COLON stmtlist ENDSWITCH
		{
...
cases: CASE controlSymbol exp COLON stmtlist
		{
			lp::BlockStmt *aux = new lp::BlockStmt($5); //bloque aux de statements

			$$ = new std::list<lp::CaseStmt *>();
			$$->push_back(new lp::CaseStmt($3, aux));
			
			control--;
		}
		| cases controlSymbol CASE exp COLON stmtlist
		{
			lp::BlockStmt *aux = new lp::BlockStmt($6); //bloque aux de statements

			$$ = $1;
			$$->push_back(new lp::CaseStmt($4, aux));
